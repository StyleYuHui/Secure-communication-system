//
// Created by PaperPlane on 2025/6/9.
//
#include "crypto/AES.h"

const unsigned char AES::sbox[256] = {
        
};

const unsigned char AES::inv_sbox[256] = {
        
};

const unsigned char AES::Rcon[11] = {
        0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36
};
std::vector<unsigned char> AES::keyExpansion(const std::vector<unsigned char>& key) {
    std::vector<unsigned char> expandedKey(176);
    unsigned char temp[4];

    for (int i = 0; i < 16; ++i) {
        expandedKey[i] = key[i];
    }

    for (int i = 16; i < 176; i += 4) {
        for (int j = 0; j < 4; ++j) {
            temp[j] = expandedKey[i + j - 4];
        }
        if (i % 16 == 0) {
            unsigned char t = temp[0];
            temp[0] = temp[1];
            temp[1] = temp[2];
            temp[2] = temp[3];
            temp[3] = t;

            for (int j = 0; j < 4; ++j) {
                temp[j] = sbox[temp[j]];
            }

            temp[0] ^= Rcon[i/16];
        }

        for (int j = 0; j < 4; ++j) {
            expandedKey[i + j] = expandedKey[i + j - 16] ^ temp[j];
        }
    }

    return expandedKey;
}

void AES::subBytes(unsigned char state[4][4]) {
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
            state[i][j] = sbox[state[i][j]];
        }
    }
}

void AES::shiftRows(unsigned char state[4][4]) {
    unsigned char temp = state[1][0];
    state[1][0] = state[1][1];
    state[1][1] = state[1][2];
    state[1][2] = state[1][3];
    state[1][3] = temp;

    std::swap(state[2][0], state[2][2]);
    std::swap(state[2][1], state[2][3]);

    temp = state[3][3];
    state[3][3] = state[3][2];
    state[3][2] = state[3][1];
    state[3][1] = state[3][0];
    state[3][0] = temp;
}

unsigned char AES::gmul(unsigned char a, unsigned char b) {
    unsigned char p = 0;
    for (int i = 0; i < 8; ++i) {
        if (b & 1) p ^= a;
        bool carry = a & 0x80;
        a <<= 1;
        if (carry) a ^= 0x1B;
        b >>= 1;
    }
    return p;
}

void AES::mixColumns(unsigned char state[4][4]) {
    unsigned char temp[4];
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
            temp[j] = state[j][i];
        }

        state[0][i] = gmul(0x02, temp[0]) ^ gmul(0x03, temp[1]) ^ temp[2] ^ temp[3];
        state[1][i] = temp[0] ^ gmul(0x02, temp[1]) ^ gmul(0x03, temp[2]) ^ temp[3];
        state[2][i] = temp[0] ^ temp[1] ^ gmul(0x02, temp[2]) ^ gmul(0x03, temp[3]);
        state[3][i] = gmul(0x03, temp[0]) ^ temp[1] ^ temp[2] ^ gmul(0x02, temp[3]);
    }
}

void AES::addRoundKey(unsigned char state[4][4], const unsigned char* roundKey) {
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
            state[j][i] ^= roundKey[i * 4 + j];
        }
    }
}

void AES::invSubBytes(unsigned char state[4][4]) {
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
            state[i][j] = inv_sbox[state[i][j]];
        }
    }
}

void AES::invShiftRows(unsigned char state[4][4]) {
    unsigned char temp = state[1][3];
    state[1][3] = state[1][2];
    state[1][2] = state[1][1];
    state[1][1] = state[1][0];
    state[1][0] = temp;

    std::swap(state[2][0], state[2][2]);
    std::swap(state[2][1], state[2][3]);

    temp = state[3][0];
    state[3][0] = state[3][1];
    state[3][1] = state[3][2];
    state[3][2] = state[3][3];
    state[3][3] = temp;
}

void AES::invMixColumns(unsigned char state[4][4]) {
    unsigned char temp[4];
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
            temp[j] = state[j][i];
        }

        state[0][i] = gmul(0x0e, temp[0]) ^ gmul(0x0b, temp[1]) ^ gmul(0x0d, temp[2]) ^ gmul(0x09, temp[3]);
        state[1][i] = gmul(0x09, temp[0]) ^ gmul(0x0e, temp[1]) ^ gmul(0x0b, temp[2]) ^ gmul(0x0d, temp[3]);
        state[2][i] = gmul(0x0d, temp[0]) ^ gmul(0x09, temp[1]) ^ gmul(0x0e, temp[2]) ^ gmul(0x0b, temp[3]);
        state[3][i] = gmul(0x0b, temp[0]) ^ gmul(0x0d, temp[1]) ^ gmul(0x09, temp[2]) ^ gmul(0x0e, temp[3]);
    }
}

std::vector<unsigned char> AES::pad(const std::string& data) {
    std::vector<unsigned char> result(data.begin(), data.end());
    unsigned char padLen = 16 - (result.size() % 16);
    for (unsigned char i = 0; i < padLen; ++i) {
        result.push_back(padLen);
    }
    return result;
}

std::string AES::unpad(const std::vector<unsigned char>& data) {
    if (data.empty()) return "";
    unsigned char padLen = data.back();
    if (padLen > 16 || padLen == 0) return "";

    if (data.size() < padLen) return "";
    for (size_t i = data.size() - padLen; i < data.size(); ++i) {
        if (data[i] != padLen) return "";
    }

    return std::string(data.begin(), data.end() - padLen);
}

void AES::encryptBlock(unsigned char state[4][4], const unsigned char* roundKeys) {
    addRoundKey(state, roundKeys);

    for (int round = 1; round <= 9; ++round) {
        subBytes(state);
        shiftRows(state);
        mixColumns(state);
        addRoundKey(state, roundKeys + round * 16);
    }

    subBytes(state);
    shiftRows(state);
    addRoundKey(state, roundKeys + 160);
}

void AES::decryptBlock(unsigned char state[4][4], const unsigned char* roundKeys) {
    addRoundKey(state, roundKeys + 160);

    for (int round = 9; round >= 1; --round) {
        invShiftRows(state);
        invSubBytes(state);
        addRoundKey(state, roundKeys + round * 16);
        invMixColumns(state);
    }

    invShiftRows(state);
    invSubBytes(state);
    addRoundKey(state, roundKeys);
}

std::string AES::encrypt(const std::string& msg, const std::string& key) {
    if (key.size() != 16) {
        throw std::invalid_argument("Key must be 16 bytes long");
    }

    std::vector<unsigned char> padded = pad(msg);
    std::vector<unsigned char> roundKeys = keyExpansion(std::vector<unsigned char>(key.begin(), key.end()));
    std::vector<unsigned char> ciphertext;

    for (size_t i = 0; i < padded.size(); i += 16) {
        unsigned char block[4][4];

        for (int row = 0; row < 4; ++row) {
            for (int col = 0; col < 4; ++col) {
                block[row][col] = padded[i + row * 4 + col];
            }
        }

        encryptBlock(block, roundKeys.data());

        for (int row = 0; row < 4; ++row) {
            for (int col = 0; col < 4; ++col) {
                ciphertext.push_back(block[row][col]);
            }
        }
    }

    return std::string(ciphertext.begin(), ciphertext.end());
}

std::string AES::decrypt(const std::string& ciphertext, const std::string& key) {
    if (key.size() != 16) {
        throw std::invalid_argument("Key must be 16 bytes long");
    }

    if (ciphertext.size() % 16 != 0) {
        throw std::invalid_argument("Ciphertext length must be multiple of 16 bytes");
    }

    std::vector<unsigned char> data(ciphertext.begin(), ciphertext.end());
    std::vector<unsigned char> roundKeys = keyExpansion(std::vector<unsigned char>(key.begin(), key.end()));
    std::vector<unsigned char> plaintext;

    for (size_t i = 0; i < data.size(); i += 16) {
        unsigned char block[4][4];

        for (int row = 0; row < 4; ++row) {
            for (int col = 0; col < 4; ++col) {
                block[row][col] = data[i + row * 4 + col];
            }
        }

        decryptBlock(block, roundKeys.data());

        for (int row = 0; row < 4; ++row) {
            for (int col = 0; col < 4; ++col) {
                plaintext.push_back(block[row][col]);
            }
        }
    }
    return unpad(plaintext);
}


